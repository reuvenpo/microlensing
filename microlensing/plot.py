...
class Plot:
    ...
    def heatmap_corner_plot(
        ...
        fig_size = 8
        self.fig, self.axes_dict = plt.subplot_mosaic(
            mosaic,
        ...
        for combo_name, combo in axis_combos:
            x, y = combo
            axes = self.axes_dict[combo_name]
            ...
            contour_axes = [numpy.ravel(data_axes[axis]) for axis in combo]
            data_index = tuple(slice(None) if axis in combo else center[axis] for axis in range(dims))
            contour_data = numpy.array(data[data_index]).reshape((resolution, resolution))
            ...
            x0,x1 = axes.get_xlim()
            y0,y1 = axes.get_ylim()
            axes.set_aspect(abs(x1 - x0) / abs(y1 - y0), adjustable='box')
        self.fig.colorbar(contour_set, cax=self.axes_dict["colorbar"])
